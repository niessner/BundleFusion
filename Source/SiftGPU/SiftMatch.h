...
public:
	...
	//desctructor
	SIFTGPU_EXPORT  ~SiftMatchGPU();

	SIFTGPU_EXPORT void InitSiftMatch();
	...
	SIFTGPU_EXPORT  void SetDescriptorsFromCPU(int index, int num, const float* descriptors, int id  = -1);
	//Option 2 unsigned char descriptors. They must be already normalized to 512
	SIFTGPU_EXPORT  void SetDescriptorsFromCPU(int index, int num, const unsigned char * descriptors, int id = -1);
	//unsigned char descriptors. They must be already normalized to 512
	SIFTGPU_EXPORT  void SetDescriptors(int index, int num, unsigned char* d_descriptors, int id = -1);
	...
	//SIFTGPU_EXPORT  int  GetSiftMatch(
	...
	SIFTGPU_EXPORT  void  GetSiftMatch(
		int max_match,	// the length of the match_buffer.
	...
	SIFTGPU_EXPORT  void SetFeautreLocation(int index, const float* locations, int gap = 0);
