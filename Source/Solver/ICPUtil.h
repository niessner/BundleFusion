...
inline __device__ float bilinearInterpolationFloatNoChecks(float x, float y, const float* d_input, unsigned int imageWidth, unsigned int imageHeight)
{
...
inline __device__ float2 bilinearInterpolationFloat2NoChecks(float x, float y, const float2* d_input, unsigned int imageWidth, unsigned int imageHeight)
{
	const int2 p00 = make_int2(floor(x), floor(y));
	const int2 p01 = p00 + make_int2(0.0f, 1.0f);
	const int2 p10 = p00 + make_int2(1.0f, 0.0f);
